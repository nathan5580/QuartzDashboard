...
public sealed class DashboardAssetTests(TestWebAppFactory factory)
{
    ...
    {
        using var response = await _client.GetAsync("/quartz/app.css");
    ...
    {
        using var response = await _client.GetAsync("/quartz/app.js");
    ...
    {
        using var response = await _client.GetAsync("/quartz/fonts/inter-latin.woff2");
    ...
    {
        var customFactory = factory.WithScenario(options => options.UseSystemFonts = true);
